...
var panda_lang_p007 = panda_zhcn ? '上一页' : 'Prev';
var panda_lang_p008 = panda_zhcn ? '下一页' : 'Next';
    ...
    xhr.onreadystatechange = function (e) {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            prev.forEach(function (value) {
                var preg = value.match(/https:\/\/exhentai\.org\/s\/(\w+)\/(\d+)-(\d+)/);
                info[preg[3]] = preg[1];
            });
            exec(info);
        }
...
function panda_showlist(prevOrNext) {
    ...
    var panda_pagetote = onePageSize;
...
function panda_plusfunc() {
    ...
    pagenavi.innerHTML = '<div id="panda_pagenavi" class="gm" style="text-align:center;"><h3>' + pagenavihtml + '</h3></div>';
    gtbn.parentNode.insertBefore(pagenavi, gtbn);
};
if (document.getElementById('panda_plus')) {
    console.log('exist');
} else if (document.domain != 'exhentai.org') {
    if (confirm(panda_lang_a002)) {
        window.location.href = 'https://exhentai.org/favicon.ico';
    }
} else if (document.getElementById('gdt')) {
    panda_plusfunc();
} else {
    panda_leapover(panda.getAttribute('exkey'));
};
